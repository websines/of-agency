...
const page = () => {
  ...
  const checkCredExists = async () => {
    const { data, error } = await supabase
      .from("user_creds")
      .select()
      .eq("id", user?.id);

    const credsBool = !error && data.length > 0;

    setCredsExists(credsBool);
  };
          ...
          <Card>
            ...
            <CardContent>
              <CardDescription className="text-md font-medium">
                Please enter your onlyfans credentials by clicking the button
                above to continue! OR if you have already entered please try to
                reload the page
              </CardDescription>
            </CardContent>
          </Card>
